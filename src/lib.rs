...
// trait ToNapiValue {
//     fn to_napi_value(env: napi::sys::napi_env, val: Self) -> napi::Result<napi::sys::napi_value>
// }

// impl ToNapiValue for HashMap<EsVersion, bool> {
//     unsafe fn to_napi_value(env: napi::sys::napi_env, val: Self) -> napi::Result<napi::sys::napi_value> {
//         let mut obj = napi::Env::from_raw(env).create_object()?;;
//         // Convert `es_versions` HashMap to NapiValue and set it in the object
//         for (key, value) in val {
//             obj.set_named_property(&key, format!("{:?}", value))?;
//         }
...
pub fn detect(options: ParseOptions) -> Result<DetectResult, napi::Error> {
    let ParseOptions { filename, code, browserslist, target } = options;
